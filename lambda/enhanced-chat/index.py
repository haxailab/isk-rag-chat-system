...
- **重要：参照件数の正確な表記** Knowledge Base文書を実際に回答に活用した場合のみ参照件数に含めること。質問と無関係な文書が検索結果に含まれていても、回答に使わなかった文書は件数に含めないこと。
- Knowledge Base文書が質問と無関係な場合は「ISK社内Knowledge Baseに関連する情報は見つかりませんでした」と正直に伝え、一般知識から回答すること。
- 回答の最後に「※この回答はISK社内Knowledge Base（{unique_kb_count}件の文書）、セッション内ファイル（{len(session_docs)}件）、インターネット検索（{len(internet_docs_filtered)}件）を参照して生成されました」と明記する。ただし実際に活用した件数のみ記載すること。
{f'- **注意：関連資料がさらに{remaining_docs}件あり、全てを参照しきれていません。** より具体的な質問をしていただくと、より正確な回答が可能です。例えば、特定の製品名、実験条件、日付などを含めてください。' if remaining_docs > 0 else (f'- 参照した{len(top_documents)}件の文書に加え、Knowledge Baseには関連する可能性のある文書がさらにあります。より詳細な情報が必要な場合は、具体的なキーワードで再度ご質問ください。' if total_raw_kb > len(top_documents) else '')}"""
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    # CORS Preflight対応（API Gateway / Function URL 両対応）
    ...
    # 認証情報取得（Cognito authorizer または リクエストボディのトークンから）
    auth_info = event.get('requestContext', {}).get('authorizer', {})
    user_sub = auth_info.get('claims', {}).get('sub', '')
    user_name = auth_info.get('claims', {}).get('cognito:username', '')

    # 認証なしエンドポイントの場合、リクエストボディの _auth_token からデコード
    if not user_name:
        try:
            body_raw = json.loads(event.get('body', '{}'))
            auth_token = body_raw.get('_auth_token', '')
            logger.info(f"Auth token present: {bool(auth_token)}, token length: {len(auth_token) if auth_token else 0}")
            if auth_token:
                ...
                if len(token_parts) >= 2:
                    ...
                    decoded = json.loads(b64.b64decode(payload).decode('utf-8'))
                    logger.info(f"Token decoded keys: {list(decoded.keys())}")
                    user_name = decoded.get('cognito:username', '')
    ...
    # リクエストボディ解析（API Gateway / Function URL 両対応）
    try:
        if 'body' in event and event['body']:
            body = json.loads(event['body'])
        else:
            body = event  # Function URL の場合
...
技術的な質問には詳細かつ正確に回答し、ISKの業務をサポートしてください。""",
            'source_links': [],
            'is_hybrid_response': False,
            'model_description': f"Claude Sonnet（一般知識・KB検索エラー: {str(e)[:100]}...）"
        }
    ...
    try:
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'details': f'Claude Sonnet呼び出しエラー: {str(e)}',
                'requestId': context.aws_request_id
            }, ensure_ascii=False)
        }